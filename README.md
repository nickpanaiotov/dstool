...
> **Важно:** повечето КЕП карти излагат **няколко слота**, затова `sign` **изисква `--slot`**.
> Първо изпълнете `list-keys`, за да видите слотовете и да изберете този с квалифицирания ключ
> (отбелязан с `non-repudiation`). Само ако картата има един-единствен слот, `--slot` може да се
> пропусне.

```sh
# 1) Намерете слота с квалифицирания ключ (вж. „Избор на слот" по-долу)
dstool list-keys

# 2) Прикачен подпис със слот 0 -> zayavlenie.p7m
dstool sign --slot 0 zayavlenie.pdf

# 3) Отделен подпис -> zayavlenie.p7s
dstool sign --slot 0 --format detached zayavlenie.pdf

# Пълен пример: слот, драйвер, ключ, алгоритъм и изходен файл
dstool sign --slot 0 --format detached \
...
```sh
DSTOOL_PIN=123456 dstool sign --slot 0 --format detached doklad.xml
```
